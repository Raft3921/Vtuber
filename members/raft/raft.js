...
for (const name of files) {
  ...
  for (let i = 0; i < d.data.length; i += 4) {
    ...
    } else if (name === "eyebrows" && b > r * 1.05 && d.data[i + 3]) {
...
try {
  ...
  alert(
    "設定ファイルを読み込めませんでした。\n" +
      "config/raft-all-settings.json と server.mjs を確認してください。\n\n" +
      err.message,
  );
}
...
function drawHairGroup(group, x, y, now, hit = false) {
  ...
  for (const i of ordered) {
    ...
    ctx.translate(px + x + 38 + a.x, py + y + groupY + a.y);
...
function downloadJson() {
  ...
  a.download = "raft-all-settings.json";
  a.click();
