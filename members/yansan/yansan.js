...
try {
  ...
  alert(
    "設定ファイルを読み込めませんでした。\n" +
      "config/yansan-all-settings.json と server.mjs を確認してください。\n\n" +
      err.message,
  );
}
...
function drawBrows(kind, x, y) {
  const gap = (mapping.layout.browGap - 204) / 2,
    ...
    slope = baseTilt + (kind === "frown" ? 4 : kind === "raised" ? -4 : 0),
    arch = kind === "raised" ? -21 : kind === "frown" ? -11 : -16,
    leftOuter = 451 - gap - 627,
    leftInner = 559 - gap - 627,
    rightInner = 695 + gap - 627,
    rightOuter = 803 + gap - 627;
  ctx.save();
  ...
  ctx.lineJoin = "round";
  ctx.beginPath();
  ctx.moveTo(leftOuter, 2);
  ctx.quadraticCurveTo((leftOuter + leftInner) / 2, arch, leftInner, slope + 2);
  ctx.moveTo(rightInner, slope + 2);
  ctx.quadraticCurveTo((rightInner + rightOuter) / 2, arch, rightOuter, 2);
  ctx.stroke();
  ctx.restore();
}
...
function downloadJson() {
  ...
  a.download = "yansan-all-settings.json";
  a.click();
