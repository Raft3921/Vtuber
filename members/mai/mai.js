...
function removeConnectedGreenBackground(canvas) {
  ...
  let head = 0,
    tail = 0;
  const isBackgroundGreen = (pixel) => {
    const i = pixel * 4,
      r = data[i],
      green = data[i + 1],
      b = data[i + 2];
    // Mai's clothes are green too.  Only the vivid studio-green backdrop is
    // eligible; darker garment greens must remain opaque even at the canvas edge.
    return (
      data[i + 3] > 0 &&
      green > 205 &&
      r < 90 &&
  ...
  const enqueue = (pixel) => {
    if (visited[pixel] || !isBackgroundGreen(pixel)) return;
    visited[pixel] = 1;
    queue[tail++] = pixel;
  };
...
function removeAllGreenBackground(canvas) {
  const g = canvas.getContext("2d", { willReadFrequently: true });
  const image = g.getImageData(0, 0, canvas.width, canvas.height);
  const { data } = image;
  for (let i = 0; i < data.length; i += 4) {
    const r = data[i],
      green = data[i + 1],
      b = data[i + 2];
    if (green > 145 && green > r * 1.55 && green > b * 1.55) data[i + 3] = 0;
  }
  g.putImageData(image, 0, 0);
}
for (const name of files) {
  ...
  g.drawImage(img, 0, 0);
  // Shared facial atlases are green-screen sprites, matching Raft's renderer.
  // Mai's member-specific art contains green clothes, so only its edge-connected
  // working background may be removed.
  if (sharedFiles.has(name)) removeAllGreenBackground(c);
  else if (name === "base" || name === "irises-v2")
    removeConnectedGreenBackground(c);
  art[name] = c;
}
...
const hairPieceDefs = [
  {
    id: "back-main",
    label: "後ろ髪",
    img: art["hair-back-generated-v14"],
    group: "back",
    pivot: [627, 365],
    weight: 0.55,
  },
  {
    id: "upper-main",
    label: "上髪",
    img: art["hair-upper-generated-v14"],
    group: "upper",
    pivot: [627, 350],
    weight: 0.8,
  },
  {
    id: "front-main",
    label: "前髪",
    img: art["hair-front-generated-v14"],
    group: "front",
    pivot: [627, 390],
    weight: 1.05,
  },
...
async function loadAllSettings() {
  ...
  return { mapping: loadedMapping, hairAdjustments: loadedHair };
}
...
try {
  ...
  alert(
    "設定ファイルを読み込めませんでした。\n" +
      "config/mai-all-settings.json と server.mjs を確認してください。\n\n" +
      err.message,
  );
}
...
function drawHairSeam(x, y) {
  // The coherent master has no synthetic seam to cover.
}
...
function render(now) {
  ...
  const idleX = Math.sin(now * 0.00073) * 1.8,
    ...
    // The regenerated Mai base shares Raft's canvas registration.
    characterY = y;
...
function downloadJson() {
  ...
  a.download = "mai-all-settings.json";
  a.click();
...
$("resetPart").onclick = () => {
  if (hairAdjustments[selectedHairId]) {
    ...
    const def = facePartDefs[selectedHairId],
      source = def.visual ? mapping.visual : mapping.layout,
      defaults = def.visual ? defaultVisual : defaultLayout;
