...
const hairPieceDefs = [
  ...
  },
  {
    id: "upper-generated",
    label: "上の髪＋尖り",
    img: art["hair-upper-generated-v17"],
    group: "upper",
    pivot: [625, 305],
    weight: 0.78,
  },
...
let hairAdjustments = makeDefaultHairAdjustments(),
  selectedHairId = "front-generated",
  highlightUntil = 0;
const defaultForeheadEyes = {
  x: 0,
  y: 0,
  gap: 104,
  scale: 1,
...
let foreheadEyes = { ...defaultForeheadEyes };
...
async function loadAllSettings() {
  ...
  return {
    ...
    foreheadEyes: { ...defaultForeheadEyes, ...(raw.foreheadEyes || {}) },
  };
}
...
try {
  ...
  foreheadEyes = loaded.foreheadEyes;
} catch (err) {
  console.error(err);
  ...
  alert(
    "設定ファイルを読み込めませんでした。\n" +
      "config/moron-all-settings.json と server.mjs を確認してください。\n\n" +
      err.message,
  );
}
...
function drawForeheadEyes(stage, x, y) {
  const frameIndex = Math.max(0, Math.min(5, stage));
  for (let side = 0; side < 2; side++) {
    const part = foreheadEyeParts[frameIndex][side],
      bounds = foreheadEyeBounds[frameIndex][side],
      cx = 627 + x + foreheadEyes.x + (side ? 1 : -1) * foreheadEyes.gap / 2,
      cy = 382 + y + foreheadEyes.y;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate((foreheadEyes.rotation * Math.PI) / 180);
    ctx.scale(foreheadEyes.scale, foreheadEyes.scale);
    anchored(part, bounds, 0, 0, 70);
    ctx.restore();
  }
}
...
function drawFacePartHighlight(g, id, x, y, now) {
  ...
  if (id === "__eye") {
    ...
    ellipse(627 + mapping.layout.eyeGap / 2, mapping.layout.eyeY, 82, 58);
  } else if (id === "__foreheadEye") {
    const rx = 45 * foreheadEyes.scale,
      ry = 35 * foreheadEyes.scale;
    ellipse(627 + foreheadEyes.x - foreheadEyes.gap / 2, 382 + foreheadEyes.y, rx, ry);
    ellipse(627 + foreheadEyes.x + foreheadEyes.gap / 2, 382 + foreheadEyes.y, rx, ry);
  } else if (id === "__iris") {
...
function render(now) {
  ...
  outlinedLayer(() => {
    ...
    drawForeheadEyes(blinkFrame, x, y);
...
function downloadJson() {
  ...
  a.download = "moron-all-settings.json";
  a.click();
...
adjustCanvas.addEventListener("pointerdown", (event) => {
  ...
  const foreheadCy = 382 + foreheadEyes.y,
    foreheadLeft = 627 + foreheadEyes.x - foreheadEyes.gap / 2,
    foreheadRight = 627 + foreheadEyes.x + foreheadEyes.gap / 2;
  if (
    Math.hypot((x - foreheadLeft) / (45 * foreheadEyes.scale), (y - foreheadCy) / (35 * foreheadEyes.scale)) <= 1 ||
    Math.hypot((x - foreheadRight) / (45 * foreheadEyes.scale), (y - foreheadCy) / (35 * foreheadEyes.scale)) <= 1
  ) return selectAdjustPart("__foreheadEye");
...
$("resetPart").onclick = () => {
  if (hairAdjustments[selectedHairId]) {
    ...
    const def = facePartDefs[selectedHairId],
      source = def.source ? def.source() : def.visual ? mapping.visual : mapping.layout,
      defaults = def.source ? defaultForeheadEyes : def.visual ? defaultVisual : defaultLayout;
